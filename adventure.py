...
def create_room(name, description, items, ends_game=False):
    """
...
##
classroom = create_room("classroom", "You're in a lecture hall, for some reason.", items = ['notepad'])
hallway = create_room("hallway", "This is a hallway with many locked doors.", items = ['book'])
staircase = create_room("staircase", "The staircase leads downward.", items = [])
outside = create_room("outside", "You've escaped! It's cold out.", items = [], ends_game=True)
gym = create_room("gym", "You are in the gym." , items = ['basketball'])
bathroom = create_room("bathroom", "You are in the bathroom, theres an unmarked switch, type flip if you want to flip the switch." , items = ['crowbar'])
##
...
create_exit(hallway, gym, "A door with the word gym is open.")
...
create_exit(staircase, bathroom, "It is dark and damp... you are in the men's restroom.")
